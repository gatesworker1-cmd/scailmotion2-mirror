...
pip install --no-cache-dir --no-deps -r /ComfyUI/requirements.txt
while read -r name url sha; do
  clone "/ComfyUI/custom_nodes/$name" "$url" "$sha"
  [ -f "/ComfyUI/custom_nodes/$name/requirements.txt" ] && pip install --no-cache-dir --no-deps -r "/ComfyUI/custom_nodes/$name/requirements.txt"
done < /tmp/nodes.txt
